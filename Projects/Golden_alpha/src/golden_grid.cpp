...
namespace Golden {
 ...
 void Cell::add(C_Object* object) {
    if (object) {objects.push_back(object); object->cells.push_back(index); types.push_back(object->type); solid=true;}
    }
...
void Cell::remove(C_Object* object) {
    ...
    std::string oType = object->type;
    int typeC = 0;
    int totalC = 0;
    for(auto o : objects)
    {
        totalC++;
        if(o->type==oType) typeC++;
    }
...
std::vector<C_Object*> Cell::getType(std::string type) {   
    ...
    for(auto o : objects)
    if(o->type==type) {toReturn.push_back(o);}
