...
class G_Instance : virtual public C_Object,virtual public Instanced
{
public:
    G_Instance() {}
    ~G_Instance() = default;
...
void genInstances(Scene& scene,Grid& grid,int instances,std::shared_ptr<Model> model,float radius,std::string type)
{
    std::vector<glm::mat4> instanceTransforms;
    instanceTransforms.reserve(instances);
    model->instance=true;
    ...
    for (int i = 0; i < instances; ++i) {
        std::shared_ptr<G_Instance> object = std::make_shared<G_Instance>();
        object->ID=i;
        object->type=type;
        ...
        float y = 0.0f;
        scene.add(object);
        grid.getCell(vec3(x,y,z)).add(object.get());
...
void genInstancesFert(Scene& scene,Grid& grid,int instances,std::shared_ptr<Model> model,float radius,std::string type)
{
    std::vector<glm::mat4> instanceTransforms;
    instanceTransforms.reserve(instances);
    model->instance=true;
    scene.setupInstance(type,model);
    std::vector<std::shared_ptr<G_Instance>> objects;
    objects.reserve(instances);
    ...
    for (int i = 0; i < instances; ++i) {
        ...
        float z = randf(-radius, radius);
        float y = 0.0f;
         while(grid.getCell(vec3(x,y,z)).fertility<0.3f)
        {
            x = randf(-radius, radius); 
            z = randf(-radius, radius); 
        }
        scene.add(object);
        grid.getCell(vec3(x,0,z)).add(object.get());
        for(auto c : grid.cellsAroundUltraFast(vec3(x,y,z),0.4f))
        c->add(object.get());
    ...
    //     o->updateCells(0.8f);
    // }
    // for(auto c : grid->cellsAroundUltraFast(getPosition(),radius))
    // c->add(this);
}
