...
g_ptr<Quad> make_button(std::string slot,const vec4& color) {
    ...
    q->setDepth(0.6f);
    q->setPosition({0,0});
    q->addScript("onHover",[q,color](ScriptContext& ctx){
        q->setColor(color*vec4(1.8f,1.8f,1.8f,1));
    });
    q->addScript("onUnHover",[q,color](ScriptContext& ctx){
        q->setColor(color);
    });
    q->addScript("onPress",[q,slot,color](ScriptContext& ctx){
        q->setColor(color*vec4(0.5,0.5,0.5,1));
    });
    q->addScript("onRelease",[q,color](ScriptContext& ctx){
        q->setColor(color);
    });
...
g_ptr<Quad> make_toggle_button(std::string slot,std::function<void(bool)> on_press,const vec4& color) {
    g_ptr<Quad> q = make_button(slot,color);
    q->addScript("onPress",[q,slot,on_press,color](ScriptContext& ctx){
...
static g_ptr<Crumb> randcrumb() {
     g_ptr<Crumb> seed = scene->create<Crumb>("Crumb");
...
static float min_match = 0.001f;

struct Instance : public Object {
    Instance() {
        add<std::function<g_ptr<Quad>()>>("gui_func", [this](){
            g_ptr<Quad> panel = scene->create<Quad>("gui_element");
            gui_panel = panel;
            vec4 color(0.2, 0.2, 0.2, 1);
            panel->setColor(color);      
            auto title = twig->makeText(form->dtype + std::to_string(form->ID));
            panel->addChild(title);                 
            make_vstack(panel, 5.0f);
            panel->updateTransform();
            autosize(panel,10.0f);
            panel->joint = nullptr;
            panel->physicsJoint = [panel,this,title](){
                panel->setCenter(scene->worldToScreen(form->getPosition()));
                float cd = scene->camera.getPosition().distance(form->getPosition());
                cd = std::max(cd, 0.001f);
                float s = 1.0f / cd;
                s = std::clamp(s, 0.02f, 100.0f);
                s*=500.0f;
                title->setScale(vec2(s,s*1.36));
                autosize(panel,10.0f);
                return true;
            };
            // if(vantage) {
            //     auto vantage_panel = make_panel(g_ptr<c_node>(vantage));
            //     auto pointer_line = make_pointer_line(vantage_panel);
            //     pointer_line->setColor(vec4(1,0,0,1));
            //     panel->addChild(pointer_line);
            // }
...
struct c_node : public Object {
    c_node(std::string _label = "UNDEFINED") : label(_label) {
        ...
        add<std::function<g_ptr<Quad>()>>("gui_func",[this](){
            ...
            vec4 color(0.2, 0.2, 0.2, 1);
            panel->setColor(color);    
            color = color+vec4(0.4f,0,0,0);   //To make the buttons more red-like     
            auto title = twig->makeText(label);
            panel->addChild(title);
                        
            auto children_text = twig->makeText("Children: " + std::to_string(children.length()));
            panel->addChild(children_text);
        
            g_ptr<Quad> tbutton = make_toggle_button("Instances: " + std::to_string(instances.length()),[panel,this](bool toggle){
                if(toggle) {
                    for(auto c : instances) {
                        //auto instance_pointer_line = make_pointer_line(c->form);
                        auto instance_panel = make_panel(c);
                        //panel->addChild(instance_pointer_line);
                        panel->addChild(instance_panel);
                    }
                } else {
                    for(auto c : instances) {
                        if(c->gui_panel&&c->gui_panel->isActive()) {
                            for(auto child : c->gui_panel->allChildren()) scene->recycle(child);
            ...
            auto mat_str = twig->makeText(archetype->to_string());
            mat_str->setPosition(vec2(panel->getRight().x(),panel->getTop().y()));
            mat_str->physicsJoint = [mat_str,panel](){
                mat_str->setPosition(vec2(panel->getRight().x(),panel->getTop().y()));
                return true;
            };
            mat_str->scaleBy(0.5f,0.5f*1.136f);
            panel->addChild(mat_str);

            for(auto c : mat_str->allChildren()) c->hide();
            mat_str->hide();

            auto mat_btn = scene->create<Quad>("gui_element");
            mat_btn->scale({20.0f,20.0f});
            mat_btn->setColor(vec4(1,0,0,1));
            mat_btn->setPosition(vec2(panel->getRight().x()-20.0f,panel->getTop().y()));
            mat_btn->physicsJoint = [mat_btn,panel](){
                mat_btn->setPosition(vec2(panel->getRight().x()-20.0f,panel->getTop().y()));
                return true;
            };
            mat_btn->addScript("onPress",[mat_btn,mat_str](ScriptContext& ctx){
                mat_btn->opt_flag = !mat_btn->opt_flag;
                if(mat_btn->opt_flag) {
                    for(auto c : mat_str->allChildren()) c->show();
                    mat_str->show();
                } else {
                    for(auto c : mat_str->allChildren()) c->hide();
                    mat_str->hide();
            ...
            panel->addChild(mat_btn);
            
            int offset = 0.0f;
            for(auto c : children) {
                g_ptr<Quad> cbutton = make_toggle_button(c->label,[c,panel](bool toggle){
                    g_ptr<Quad> p = panel;
                    ...
                    g_ptr<Quad> new_panel = make_panel(c);
                    new_panel->physicsJoint = nullptr; //Disables panel attatchment to archetype form
                    new_panel->setCenter(vec2(win.x() / 2.0f, win.y() / 2.0f));
                },color);
                cbutton->joint = [panel,cbutton,offset](){
                    cbutton->position = panel->getLeft().addY(panel->getScale().y()/2) + vec2(offset,0);
                    return true;
                };
                cbutton->rotate(atan2(1.0f,0.2f));
                offset+=40.0f;
                panel->addChild(cbutton);
            }

            offset = 0.0f;
            for(auto p : parents) {
                g_ptr<Quad> pbutton = make_toggle_button(p->label,[p,panel](bool toggle){
                    g_ptr<Quad> clear = panel;
                    while(clear->parent) clear = clear->parent;
                    for(auto toclear : clear->allChildren()) {
                        scene->recycle(toclear);
                    }
                    scene->recycle(clear);

                    g_ptr<Quad> new_panel = make_panel(p);
                    new_panel->physicsJoint = nullptr;
                    new_panel->setCenter(vec2(win.x() / 2.0f, win.y() / 2.0f));
                },color);
                pbutton->joint = [panel,pbutton,offset](){
                    pbutton->position = panel->getLeft().addY(-((panel->getScale().y()/2)+20)) + vec2(offset,0);
                    return true;
                };
                pbutton->rotate(atan2(-1.0f,0.0f));
                offset+=40.0f;
            ...
            // offset = 0.0f;
            // for(auto e : episodes) {
            //     g_ptr<Quad> ebutton = make_toggle_button(std::to_string(e->getID()),[e,panel](bool toggle){
            //         g_ptr<Quad> clear = panel;
            //         while(clear->parent) clear = clear->parent;
            //         for(auto toclear : clear->allChildren()) {
            //             scene->recycle(toclear);
            ...
            //         g_ptr<Quad> new_panel = make_panel(e);
            //         new_panel->physicsJoint = nullptr;
            //         new_panel->setCenter(vec2(win.x() / 2.0f, win.y() / 2.0f));
            //     },color);
            //     ebutton->joint = [panel,ebutton,offset](){
            //         ebutton->position = panel->getLeft().addY(-((panel->getScale().y()/2)+20)) + vec2(-ebutton->getScale().x(),offset);
            //         return true;
            //     };
            //     offset+=30.0f;
            //     panel->addChild(ebutton);
            // }

            panel->physicsJoint = [panel,this](){
                panel->setCenter(scene->worldToScreen(getPosition()));
                return true;
            };
    ...
    list<g_ptr<Episode>> episodes;

    //DEBUG ONLY METHOD
    vec3 getPosition() {
        return vec3(scene->transforms[archetype->id][3]);
    }
...
public:
    ...
    // list<g_ptr<tree_walker>> children() {
    //     list<g_ptr<tree_walker>> to_return;
    //     for(auto n : node->neighbors()) {
    //         g_ptr<tree_walker> child = scene->create<tree_walker>("tree_walker");
    //         child->node = n;
...
public:
    golden_nodenet(g_ptr<Crumb> agent_state) : state(agent_state) {
        ...
        cognitive_attention << agent_state;

        cmask = clone(IDENTITY);
    };
    ~golden_nodenet() {};
    ...
    void walk_graph(g_ptr<c_node> start, 
        ...
        while(!queue.empty()) {
            ...
            void* ptr = (void*)current->node.getPtr();
    ...
    g_ptr<c_node> match(g_ptr<c_node> start,g_ptr<Crumb> item) {
        ...
        if(!to_return||to_return==start) {
            return nullptr;
        }
        return to_return;
    }
    g_ptr<c_node> match(g_ptr<c_node> start,g_ptr<Single> item) {
        return match(start,crumbs.get(item->ID));
    }
    ...
    float desire_salience(g_ptr<Crumb> observation) {
        return evaluate(state, DOES, observation, IS);
    }
    ...
    void observe_instances(g_ptr<Single> agent,int on_frame) {
        ...
        propagate(obs_root,energy,[&observed_objects,&add_inst](g_ptr<tree_walker> walker) {
            ...
            for(int i=observed_objects.length()-1;i>=0;i--) {
                ...
                if(node->matches_category(s)<=min_match) {
        ...
        for(auto o : observed_objects) {
            ...
            for(auto c : new_cats) { //Same as above
                if(c->matches_category(o)<=min_match) { 
        ...
        float best_action_salience = 0.0f;
        Action best_action(1);
        g_ptr<Single> best_action_target = nullptr;
        for(auto instpair : add_inst) {
            g_ptr<Single> object = instpair.first;
            c_node* category = instpair.second;
            // Calculate distance (for proximity weighting)
            vec3 to_object = object->getPosition() - agent->getPosition();
            float distance = to_object.length();

            //Evaluating the archetypal actions against agent state for instinctive interaction, modulated by distance for salience
            int energy = 10;  
            // remember(category, energy, [&](g_ptr<tree_walker> walker) {
            //     g_ptr<c_node> action = walker->node;
            //     float sal = evaluate(state, DOES, action->archetype, DOES);
            //     sal *= (1.0f / (distance + 1.0f));
            //     if(sal > best_action_salience) {
            //         best_action_salience = sal;
            //         best_action = Action{3}; //Currently fixed to 3 for use, but in the future will be decied by curisoity
            //         best_action_target = object;
            //     }
            //     return true;
            // });
        }
        
        // If instinct triggered, execute action sequence
        if(best_action_target) {
            vec3 f = best_action_target->getPosition();
            execute_action_on_agent(Action{1,{f.x(),f.y(),f.z()}}, agent);
            execute_action_on_agent(Action{2}, agent);
            execute_action_on_agent(best_action, agent);
        }
    }
        ...
        for(int i = 1; i < input_size; i++) {
            ...
            if(gap_start < 0 && depth_jump > rayspace) {
                ...
                [&](g_ptr<tree_walker> walker) {
                    ...
                    if(spatial_dist<rayspace*2.0f) {
                        if(spatial_dist<closest_dist) {
                            closest_dist = spatial_dist;
                            best_node = node;
                        }
                    }
    ...
    g_ptr<c_node> find_category_for_id(int id) {
        g_ptr<Crumb> crumb = crumbs.get(id);
        g_ptr<c_node> best_match = match(obs_root,crumb);
        if(!best_match) {
            return obs_root->subnode(crumb);
        } else {
            if(best_match->matches_category(crumb)<=min_match) {
                return best_match;
            } else {
                return best_match->subnode(crumb);
            }
        }
    }

    list<g_ptr<Crumb>> gather_states() {
        ...
        ids_to_push.erase(agent_id);

        for(int i=1;i<cognitive_attention.length();i++) {
            states << ensure_imutable(cognitive_attention[i]);
            if(is_mutable(states[i])) {
                if(ids_to_push.has(states[i]->id)) {
                    states[i]->decay = 1.0f;
                    ids_to_push.erase(states[i]->id);
                } else {
                    states[i]->decay += 0.1f;
                }
            }
        ...
        list<float> cog_sals = {0.0f}; //For the first agent state, as padding.
        for(int i=1;i<states.length();i++) {
            cog_sals << (salience(states[i])/states[i]->decay);
        }

        list<std::pair<g_ptr<Crumb>, float>> id_sal;
        for(auto id : ids_to_push) {
            if(!crumbs.hasKey(id)) continue;
            g_ptr<Crumb> obs = get_crumb(id);
            float sal_boost = 0.0f;
            for(int ray = 0; ray < vis->dists.length(); ray++) {
                if(vis->cells[ray] == -1) continue;
                if(grid->cells[vis->cells[ray]].has(id)) {
                    float centrality = 1.0f - std::abs(ray - vis->num_rays/2.0f) / (vis->num_rays/2.0f);
                    float closeness = 1.0f / (vis->dists[ray] + 1.0f);
                    sal_boost = centrality * closeness * 2.0f;  // Tune multiplier
                    break;
                }
            }
            id_sal << std::make_pair(obs, salience(obs) + sal_boost);
        }
        id_sal.sort([](const auto& a,const auto& b) { return a.second > b.second; });
        for(auto [crumb,sal] : id_sal) {
            for(int i=1;i<cog_sals.length();i++) {
                if(cog_sals[i]<sal) {
                    cog_sals[i] = sal;
                    states[i] = ensure_imutable(crumb);
                    break;
                }
            }
        }
    ...
    void execute_action_on_agent(const Action& action,g_ptr<Single> agent) {
        ...
        //The idea here is to put interacted items at the front of cognitive attention
        list<g_ptr<Crumb>> after = gather_states();
        for(int i=0;i<interactions.length();i++) {
            if((i+1)>=after.length()) break;
            g_ptr<Crumb> obj_crumb = get_crumb(interactions[i]);
            if(obj_crumb!=ZERO) {
                after[i+1] = clone(obj_crumb); 
            }
        }
        if(action.part_id>1) {
            list<g_ptr<Crumb>> before;
            for(auto c : cognitive_attention) 
                before << clone(c);
            ...
            for(int i = 0; i < max_len; i++) {
                ...
                else if(i >= after.length()) { //Destoryed
                    delta = ZERO;  //For destroyed objects we want an object permenance system we can turn on and off later
                    //This will let us do more percise transferals, like having a berry be picked up and the specific berry 
                    //crumb be moved into the agent's holding slot
                }
                else { //Observe change
                    delta = clone(after[i]);
     ...
     void visualize_nodes(const list<g_ptr<c_node>>& nodes, g_ptr<c_node> highlight = nullptr) {
        ...
        for(auto cog : cognitive_attention) {
            auto marker = scene->create<Single>("cyan");
            marker->setPosition(vec3(scene->transforms[cog->id][3]).addY(3.0f));
            marker->setScale({0.2f,1.0f,0.2f});
            debug << marker;
        }
    }
...
static list<g_ptr<golden_nodenet>> crumb_managers;
...
int main() {
    ...
    scene->add_initilizer("berry",[](g_ptr<Object> obj){ 
        if(g_ptr<Single> berry = g_dynamic_pointer_cast<Single>(obj)) {
            ...
            berry_crumb->mat[LER-10][BOREDOM] = 1.1f; 
            berry_crumb->mat[LER-10][HUNGER] = 2.3f; 
            ...
            berry->addScript("onUse",[berry](ScriptContext& ctx){
                ...
                sub_mask(crumb_managers[from->TID]->state, DOES, get_crumb(berry->ID), IS);
    ...
    scene->add_initilizer("berry_bush",[](g_ptr<Object> obj){ 
        if(g_ptr<Single> bush = g_dynamic_pointer_cast<Single>(obj)) {
            ...
            bush->addScript("onUse",[bush](ScriptContext& ctx){
                ...
                sub_mask(crumb_managers[from->TID]->state, DOES, crumbs.get(bush->ID), DOES);
    ...
    scene->define("badberry",[berry_model](){
        g_ptr<Single> berry= make<Single>(berry_model);
        scene->add(berry);    
        return berry;
    });
    scene->add_initilizer("badberry",[](g_ptr<Object> obj){ 
        if(g_ptr<Single> berry = g_dynamic_pointer_cast<Single>(obj)) {
            //Cleanup
            berry->scripts.clear(); //Clear all scipts
            removeFromGrid(berry); //Remove it from grid
            ...
            berry->setPhysicsState(P_State::PASSIVE);
            berry->setScale(1.3f);
            berry->setColor(vec4(0.2f,0.2f,0.2f,1));
            addToGrid(berry);
            g_ptr<Crumb> berry_crumb = scene->create<Crumb>("Crumb"); //Intilize it with evrything
            berry_crumb->id = berry->ID;
            berry_crumb->mat[OBS][0] = 0.12f; //Different size again
            ...
            berry->addScript("onUse",[berry](ScriptContext& ctx){
                ...
                sub_mask(crumb_managers[from->TID]->state, DOES, crumbs.get(berry->ID), DOES);
    ...
    scene->define("badberry_bush",[c_model](){
        g_ptr<Single> bush= make<Single>(c_model);
        scene->add(bush);
        return bush;
    });
    scene->add_initilizer("badberry_bush",[](g_ptr<Object> obj){ 
        if(g_ptr<Single> bush = g_dynamic_pointer_cast<Single>(obj)) {
            bush->scripts.clear();
            removeFromGrid(bush);
            ...
            g_ptr<Crumb> bush_crumb = scene->create<Crumb>("Crumb"); //Intilize it with evrything
            bush_crumb->id = bush->ID;
            bush_crumb->mat[OBS][0] = 2.0f; //Signifigantly different size
    
            crumbs.put(bush->ID,bush_crumb);
    
            for(int i=0;i<randi(2,6);i++) {
                auto b = scene->create<Single>("badberry");
                b->setPosition(vec3(randf(-0.5f,0.5f),randf(1,4),randf(-0.5f,0.5f)));
                removeFromGrid(b);
                bush->addChild(b);
            }
    ...
    scene->define("agent",[&](){
        ...
        scene->add(agent);
        agent->opt_vec_3_3 = agent->getPosition();
        agent->joint = [agent](){
            agent->unlockJoint = true; agent->updateTransform(); agent->unlockJoint = false;
            for(auto i : agent->opt_idx_cache) {grid->cells[i].erase(agent->ID);}
            agent->opt_idx_cache = grid->addToGrid(agent->ID, agent->getWorldBounds());
            return false;
        };
        agent->isAnchor = true;
        return agent;
    });
    scene->add_initilizer("agent",[&](g_ptr<Object> obj){ 
        if(g_ptr<Single> agent = g_dynamic_pointer_cast<Single>(obj)) {
            agent->opt_ints.clear();
            agent->opt_floats.clear();
            crumbs.remove(agent->ID);
            addToGrid(agent);
        ...
        //   #endif
            ...
            mem->mat[CUR][CUR_OBSERVE] = 1.0f;
            mem->mat[CUR][CUR_INTERACT] = 1.0f;
            mem->mat[CUR][CUR_USE] = 1.0f; 
            mem->mat[CUR][CUR_DROP] = 1.0f; 
            mem->mat[CUR][CUR_HIT] = 1.0f; 

            mem->mat[LER][BOREDOM] = 5.0f;
            mem->mat[LER][HUNGER] = 8.0f; 
            mem->mat[LER][FATIGUE] = 1.0f;
            mem->id = agent->ID;
            
            mem->sub << ZERO; //Dominant paw
            mem->sub << ZERO; //Other paw
            mem->sub << ZERO; //Inventory 1
            mem->sub << ZERO; //Inventory 2

            crumbs.put(agent->ID,mem);

            g_ptr<golden_nodenet> memory = make<golden_nodenet>(mem);
            memory->scene = scene;
            ...
            agent->threadUpdate = [agent, i, memory, update_thread](){
                ...
                g_ptr<golden_nodenet> memory = crumb_managers[id];
                on_frame++; 
    ...
    g_ptr<golden_nodenet> mem = crumb_managers[0];
    ...
    fir->physicsJoint = nullptr;
    g_ptr<golden_nodenet> net = crumb_managers[fir->TID];
    ...
    int cam_mode = 2;
     ...
     start::run(window,d,[&]{
        ...
        if(pressed(MOUSE_LEFT)) {
            if(!grid->empty(mousepos)) {
                int clicked_id = grid->getCell(mousepos)[0];
                g_ptr<Single> clicked = scene->singles[clicked_id];
                int find_id = agents.find(clicked);
                if(find_id!=-1) {
                    g_ptr<golden_nodenet> nnet = crumb_managers[find_id];
                    g_ptr<Quad> panel = make_panel(nnet->obs_root);
                    panel->physicsJoint = nullptr;
                    panel->setCenter(vec2(win.x() / 2.0f, win.y() / 2.0f));
                    panel->addChild(make_pointer_line(clicked));
                }
            }
        }
        ...
        float gui_zoom = 1.0f;
        vec2 zoom_point = vec2(win.x() / 2.0f, win.y() / 2.0f); // Screen center
        //scene->mousePos2d();

        
        if(pressed(Q)) {
            float old_zoom = gui_zoom;
            if(shift) {
                gui_zoom *= 0.5f;
            } else {
                gui_zoom *= 2.0f;
            }
            float zoom_change = gui_zoom / old_zoom;
            for(int i = 0; i < scene->quads.length(); i++) {
                auto q = scene->quads[i];
                if(!scene->quadActive[i]) continue;
                vec2 to_quad = q->position - zoom_point;
                q->position = zoom_point + to_quad * zoom_change;
                q->scaleVec = q->scaleVec * zoom_change;
                
                q->updateTransform();
            }
        }
        vec2 pan_input = input_2d_arrows(20.0f);
        if(pan_input.length() > 0.01f) {
            for(int i = 0; i < scene->quads.length(); i++) {
                auto q = scene->quads[i];
                if(!scene->quadActive[i]) continue;
                
                q->position = q->position + pan_input;
                q->updateTransform();
            }
        }
        ...
        if(fir&&control_fir) {
            ...
            if(pressed(H)) {
                ...
                for(auto c : net->obs_root->children) {
                    print("CATEGORY: ",c->label," C_LEN: ",c->children.length());
            ...
            if(pressed(J)) {
                // print("------------\nASSESING EPISODIC MEMORY: ",net->obs_root->children.length());
                // for(auto c : net->obs_root->children) {
                //     print("CATEGORY: ",c->label," C_LEN: ",c->children.length());
                //     for(auto e : c->episodes) {
                //         print("  ACTION: ",e->getID()," ARC-HUNGER: ",e->archetype->mat[LER][HUNGER]);
                //         for(int i=0;i<e->children.length();i++) {
                //             print("   CHILD: ",e->children[i]->label,"-",e->children[i]->getID());
                //             print("   ARC-HUNGER: ",e->deltas[i]->mat[LER][HUNGER]);
                //             print("   ARC-SIZE: ",e->deltas[i]->mat[OBS][0]);
                //         }
                //     }
                // }
            }
