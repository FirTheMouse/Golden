...
static float evaluate_matmul(g_ptr<Crumb> eval, g_ptr<Crumb> against, g_ptr<Crumb> result) {
 ...
 static float evaluate_matmul(g_ptr<Crumb> eval, g_ptr<Crumb> against) {
     g_ptr<Crumb> result = scene->create<Crumb>("Crumb");
...
public:
    ...
    void form_episode(g_ptr<c_node> category, int action_id, list<g_ptr<Crumb>>& changed_crumbs, list<int>& destroyed_ids, list<g_ptr<Crumb>>& created_crumbs) {
        g_ptr<c_node> action_node = nullptr;
        for(auto ep : category->episodes) {
            if(ep->archetype->id == action_id) {
                action_node = ep;
                break;
            }
        }
        ...
        if(!action_node) {
            is_new = true;
            action_node = make<c_node>();
            category->episodes << action_node;
        } else { //If there's already an existing action node
            //Nothing for now
        }

        //Object destruction, we zero the matrix so the agent won't consider it further down the chain of reasoning to simulate destruction
        for(auto id : destroyed_ids) { 
            g_ptr<c_node> destroyed_cat = find_category_for_id(id);
            if(is_new||!action_node->children.has(destroyed_cat)) { //If a new node, or novel observation about an existing episode
                g_ptr<Crumb> zero_delta = scene->create<Crumb>("Crumb");
                zero_delta->id = id;
                zero_delta->setmat(0.0f);

                action_node->children << destroyed_cat;
                action_node->deltas << zero_delta;
            }
        }

        //Object creation is more complex than destruction, still figuiring out how to handle it exactly
        for(auto& created : created_crumbs) {
            g_ptr<c_node> created_cat = find_category_for_id(created->id);
            if(is_new||!action_node->children.has(created_cat)) {
                action_node->children << created_cat;
                action_node->deltas << created;
            }
        }

        for(auto delta : changed_crumbs) {
            if(delta->id==agent_id) {
                if(is_new) {
                    action_node->archetype = delta;
                    action_node->archetype->id = action_id;
                } else { //Need to actually track the ammount of observations for a proper averege, this won't really work
                    add_mask(action_node->archetype, LER, delta, LER);
                    scale_mask(action_node->archetype, LER, 0.5f);
                }
            } else {
                //The ID is carried forwards from our intial observation, because crumbs link to a Single form right now
                g_ptr<c_node> delta_category = find_category_for_id(delta->id);
                int child_idx = -1; //Check to see if this node already contains this category of interaction or not
                if(!is_new) { //yes, this is branching, but it stops the o(n) case on frequent updates
                    for(int i = 0; i < action_node->children.length(); i++) {
                        if(action_node->children[i] == delta_category) {
                            child_idx = i;
                            break;
                        }
                    }
                }
                if(is_new||child_idx == -1) {
                    //If multi-item interaction curiosity is added, it could be satiated here
                    action_node->children << delta_category;
                    action_node->deltas << delta;
                } else { //Avg if existing (need to track observations somehow here too if we want to be accurate)
                    g_ptr<Crumb> existing = action_node->deltas[child_idx];
                    add_mask(existing, ALL, delta, ALL);
                    scale_mask(existing, ALL, 0.5f); 
                }
            }
        }
    ...
    void execute_action_on_agent(const Action& action,g_ptr<Single> agent) {
        ...
        //This systems needs to be revised for flexibility.
        //For one, it should be easier to turn on and off, so we dont' constantly observe as moving
        //For two, we need better item to item interaction observation, rather than just gather every interaction and evluate with the same map
        //Given as we form the same episode for every interacted object, which isn't a problem when almost every interaciton is only one object
        //This is something to fix in a future refractor
        list<g_ptr<Crumb>> after = gather_states(agent);
        for(auto item : interactions) {
            g_ptr<c_node> cat = match(obs_root,item);
            if(!cat) {
                cat = obs_root->subnode(crumbs.get(item->ID));
                cat->label = item->dtype;
            }
            ...
            for(auto& [id, before_crumb] : before_map.entrySet()) {
                if(!after_map.hasKey(id)) {
                    ...
                    g_ptr<Crumb> delta = clone(before_crumb);
                    sub_mask(delta, ALL, after_map[id], ALL);
                    if(delta->absSum()>0.001f||id==agent_id) { //The minimum threshold for change between two states
            ...
            //Action paramaters should be stored as well somewhere
...
int main() {
    ...
    g_ptr<nodenet> net = crumb_managers[fir->TID];
    ...
    // net->id_to_node.put(root_form->ID,net->obs_root.getPtr());
    // crumb_managers << net;
    // agents << fir;
