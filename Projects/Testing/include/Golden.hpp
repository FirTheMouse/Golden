...
public:
    static uint32_t registerType(const std::string& name) {
    ...
    static uint32_t ID(uint32_t hash) {
        if(!hash_to_enum.hasKey(hash)) { //Horrible for performance, replace this later but keep it for security now
            print("g_register::40 Warning, key not found for enum ",hash,"!");
            return 0;
        }
    ...
    static std::string name(uint32_t ID) {
        if(!enum_to_string.hasKey(ID)) { //Horrible for performance, replace this later but keep it for security now
            print("g_register::50 Warning, key not found for enum ",ID,"!");
            return "[null]";
        }
...
#define GET_TYPE(name) g_register::ID(TYPE_ID(name))
#define TO_STRING(ID) g_register::name(ID)
