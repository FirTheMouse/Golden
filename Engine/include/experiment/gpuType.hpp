...
public:
    ...
    gpuType() {
        #ifdef __APPLE__
            allocator = create_metal_allocator();
        #else
            allocator = nullptr;  // No GPU support on this platform yet
        #endif
    ...
    ~gpuType() {
        if (allocator) {
            for (auto ptr : byte4_gpu_columns) {
                if (ptr) allocator->free(ptr);
            }
            // ADD OTHER COLUMNS EVENTUALLY!
            delete allocator;
        }
    }
    ...
    list<void*> byte1_gpu_columns;   // bool, char (1 byte)
    list<void*> byte2_gpu_columns;  // uint16_t (2 bytes)
    list<void*> byte4_gpu_columns;  // int, float (4 bytes)  
    list<void*> byte8_gpu_columns;  // double, pointers (8 bytes)
    list<void*> byte16_gpu_columns; // vec4, padded vec3 (16 bytes)
    list<void*> byte24_gpu_columns; // strings, list, medium objects (24 bytes)
    list<void*> byte32_gpu_columns; // strings on some systems (32 bytes)
    list<void*> byte64_gpu_columns; // mat4, large objects (64 bytes)
