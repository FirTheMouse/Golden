...
class P_Sim : virtual public Sim, virtual public Singleton<P_Sim>
{
...
public:
    // std::unordered_set<size_t> RIDS;
    // std::mutex RIDSmutex;
    // void collectGarbage()
    // {
    //     if(RIDS.size()>0)
    //     {
    //     std::unordered_set<size_t> LRIDS;
    //     {
    //         std::lock_guard<std::mutex> lock(RIDSmutex);
    //         std::swap(LRIDS,RIDS);
    //     }

    //     // for(size_t i=LRIDS.size()-1;i>=0;i--)
    //     // {
    //     // }

    //     //Add timing in the future just like with the task queue
    //     for(size_t uuid : LRIDS)
    //     {
    //         if(scene->objects.hasKey(uuid))
    //         {
    //             auto o = scene->getObject(uuid);
    //             print("REMOVAL ON PHYSICS THREAD FOR \n UUID=",o->UUID," ID=",o->ID);  
    //             o->remove();
    //         }
    //         else
    //         {
    //             std::cerr << "[Warn Physics 64] UUID from removal set not in Objects map!";
    //         }
    //     }
    ...
    {       
        ...
        for (size_t i = 0; i < scene->transforms.length(); ++i) {
            ...
            if(p==P_State::DETERMINISTIC)
            {
                ...
                a.elapsed = std::min(a.elapsed + sliceSpeed, a.duration);
            ...
            else if(p==P_State::ACTIVE)
            {
                ...
                pos += vec3(velocity.position * sliceSpeed * velocityScale).toGlm();
                transform[3] = glm::vec4(pos, 1.0f);
                
                // --- Rotation ---
                glm::vec3 rotVel = vec3(velocity.rotation * sliceSpeed * velocityScale).toGlm();
                ...
                // --- Scale ---
                glm::vec3 scaleChange = vec3(velocity.scale * sliceSpeed * velocityScale).toGlm();
